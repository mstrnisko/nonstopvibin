...
Linux needs a desktop secret service such as GNOME Keyring or KWallet. The app refuses Electron's plaintext fallback. GNOME may require an AppIndicator extension to show a tray icon; the View menu's quota command remains available. In a headless Ubuntu 24.04 arm64 container, the extracted AppImage app returned HTTP 000 and timed out with only D-Bus errors in its log, both without a secret service and after one GNOME Keyring retry. The arm64 AppImage runtime needs `libz.so` from `zlib1g-dev`; this does not affect the x64 artifacts we publish. A real desktop Linux session is still untested. CI builds and tests on Linux and macOS.
