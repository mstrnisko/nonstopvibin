# Agent setup for `nonstopvibin`

This is a deepsec scanning workspace. Project `nonstopvibin` was just registered
(target: `..`). Manual context setup completed on 2026-09-06. `data/nonstopvibin/INFO.md`
contains reviewed project context; AI onboarding and processing have not run.
Use the rubric below when refreshing context after architecture changes.

## What to do

1. **Read the deepsec skill.** After `bun ci`, the file is at
...
2. **Fill in `data/nonstopvibin/INFO.md`.** It's auto-injected into the AI
...
```bash
bun run deepsec scan    --project-id nonstopvibin
bun run deepsec process --project-id nonstopvibin
```
